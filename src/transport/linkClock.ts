 ...
 * ------------------------
 * Adapted from the mdrone project's `src/engine/linkClock.ts` (AGPL-3.0;
 * github.com/gdamdam/mdrone), which shares the mpump Link bridge protocol.
 * mchord only needs the grid-boundary / quantize math, so the breathing-LFO
 * helpers (nextPeakAnchor, lfoSyncedHz) from the original are omitted. The
 * remaining pure functions are unchanged apart from naming.
 *
