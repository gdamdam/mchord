...
[![tests](https://img.shields.io/badge/tests-333%20passing-2ea043)](#verification)
...
- **Sixteen play styles** — block (Hold, Pulse, Stab, Offbeat), arps (↑/↓/↕, Broken), and **Split** performances that play the chord as a low bass voice plus a moving melody/arp in a different octave (Bass + Melody, House Stab + Bass, Techno Roll, Trance Arp, Dub Skank, Synthwave Drive, Lo-Fi Broken, Garage 2-Step) — all responsive to BPM, swing, and Motion, with forward / reverse / pendulum / seeded-random playback.
...
```bash
npm run check   # typecheck + lint + 333 tests + production build
```
